...
centroid <- function(podatki) {
  ...
  x <- 0 # predpostavka o splošni legi
  
  return(c(x, y))
}

centroid.kviz <- sum(centroid(skupina))
centroid.kviz
...
# uredimo po abecednem vrstem 

for (i in 1:nrow(podatki23)) {
  vrstica <- podatki23[i, ]
  len <- length(vrstica)
  vrstica <- sort(vrstica)
  # dopolnimo vrstico do prave dolžine
  podatki23[i, ] <- c(sort(vrstica), rep(NA, len - length(vrstica))) 
}
...
pojavitev.kviz <- sum(pogoste$freq)
pojavitev.kviz
